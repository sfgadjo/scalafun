...
trait Stream[+A] {
  ...
  */
  def toList: List[A] = {
    @annotation.tailrec
    def go(s: Streamz[A], acc: List[A]): List[A] = s match {
  ...
  */
  def toListFast: List[A] = {
    ...
    @annotation.tailrec
    def go(s: Streamz[A]): List[A] = s match {
      case Cons(h,t) =>
        buf += h()
        go(t())
      case _ => buf.toList
  ...
  */
  def take(n: Int): Streamz[A] = this match {
  ...
  */
  @annotation.tailrec
  final def drop(n: Int): Streamz[A] = this match {
  ...
  */
  def takeWhile(f: A => Boolean): Streamz[A] = this match {
  ...
  */
  ...
  def takeWhile_1(f: A => Boolean): Streamz[A] =
  ...
  def append[B>:A](s: => Streamz[B]): Streamz[B] =
  ...
  def takeWhileViaUnfold(f: A => Boolean): Streamz[A] =
    unfold(this) {
      case Cons(h,t) if f(h()) => Some((h(), t()))
      case _ => None
    }

  def zipWith[B,C](s2: Streamz[B])(f: (A,B) => C): Streamz[C] =
    unfold((this, s2)) {
  ...
  def zip[B](s2: Streamz[B]): Streamz[(A,B)] =
    zipWith(s2)((_,_))
  ...
  def zipWithAll[B, C](s2: Streamz[B])(f: (Option[A], Option[B]) => C): Streamz[C] =
  ...
  */
  def startsWith[A](s: Streamz[A]): Boolean =
  ...
  */
  def tails: Streamz[Streamz[A]] =
    ...
    } append Streamz(empty)

  def hasSubsequence[A](s: Streamz[A]): Boolean =
...
case class Cons[+A](h: () => A, t: () => Streamz[A]) extends Streamz[A]
...
object Stream {
  ...
  val ones: Streamz[Int] = Streamz.cons(1, ones)

  // This is more efficient than `cons(a, constant(a))` since it's just
  // one object referencing itself.
  def constant[A](a: A): Streamz[A] = {
    lazy val tail: Streamz[A] = Cons(() => a, () => tail)
    tail
  }
   ...
   */
  def unfoldViaFold[A, S](z: S)(f: S => Option[(A, S)]): Streamz[A] =
    f(z).fold(empty[A])((p: (A,S)) => cons(p._1,unfold(p._2)(f)))

  def unfoldViaMap[A, S](z: S)(f: S => Option[(A, S)]): Streamz[A] =
