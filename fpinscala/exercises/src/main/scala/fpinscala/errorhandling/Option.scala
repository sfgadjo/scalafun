package scalafun.fpinscala.errorhandling
...
object Option {
  ...
  def sequence[A](a: List[Option[A]]): Option[List[A]] = a.foldRight[Option[List[A]]](Some(Nil))((value, opt) => map2(value, opt)(_ :: _))

  def traverse[A, B](a: List[A])(f: A => Option[B]): Option[List[B]] = sequence(a.map(value => f(value)))
}